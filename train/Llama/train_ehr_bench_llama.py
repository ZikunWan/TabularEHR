...
class EHRBenchMEDSDataCollator:
    ...
    def _tokenize_sample(self, sample: dict):
        ...
        if label_text not in self.label_to_id:
            raise ValueError(
                ...
                f"for EHR-Bench task '{self.task_name}'. Expected one of: {sorted(self.label_to_id.keys())}"
            )
...
def _load_source_dataset(data_args, sample_info_path: str, split_name: str, shuffle: bool, max_samples: Optional[int]):
    dataset = MIMICIV(
...
def main():
    ...
    rank0_print("=" * 80)
    rank0_print("EHR-Bench MEDS Llama Encoder Classifier Train")
    rank0_print("=" * 80)
