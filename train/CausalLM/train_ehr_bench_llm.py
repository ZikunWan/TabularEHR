...
def _normalize_label(raw_label) -> str:
    """Normalize labels from CSV/JSON into a canonical string form."""
...
def _augment_binary_aliases(label_to_id):
    """Allow 0/1 and yes/no style labels to map to the same binary ids."""
    aliases = {}
...
@dataclass
class EHRBenchScriptArguments(LocalLLMScriptArguments, ScriptArguments):
    data_dir: str = field(
...
def _build_label_metadata(task_name: str):
    ...
    if task_type == "multi_label_classification":
        raise NotImplementedError(
            f"Sequence classification mode does not currently support multi-label MIMIC task '{task_name}'."
        )
...
def _build_classification_dataset(
    ...
    for index in range(len(source_dataset)):
        ...
        if label not in label_to_id:
            raise ValueError(
                f"Unexpected label '{raw_label}' (normalized: '{label}') for MIMIC task '{task_name}'."
            )
...
def main():
    parser = TrlParser((EHRBenchScriptArguments, SFTConfig, ModelConfig))
