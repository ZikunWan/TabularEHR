...
def _build_label_metadata(task_name: str):
    ...
    if task_type == "multi_label_classification":
        raise NotImplementedError(
            f"EncoderLM testing does not support multi-label eICU task '{task_name}'."
        )
...
def main():
    ...
    sample_info_path = data_args.sample_info_path or os.path.join(data_args.processed_dir, "sample_info_test.json")
    candidates, label_to_id, id_to_label = _build_label_metadata(data_args.task_name)

    print("=" * 80)
    print("eICU EncoderLM Test")
    print("=" * 80)
