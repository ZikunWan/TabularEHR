...
from dataset.renji.renji_survival_dataset import (
    STAGE_SPECS,
    RenjiTacrolimusSurvivalDataset,
    build_survival_instruction,
)
from models.TableEncoder.config import LongTableEncoder1DConfig
from models.TableEncoder.query_survival import TaskQueryPiecewiseSurvivalModel
...
class DataArguments:
    ...
    patient_subset_path: Optional[str] = field(
        default="/data/zikun_workspace/code/data/patients.json"
    )
...
def main():
    ...
    dataset = RenjiTacrolimusSurvivalDataset(
        root_dir=data_args.data_dir,
        split=data_args.split,
        shuffle=False,
        patient_subset_path=data_args.patient_subset_path,
    )
    train_reference_dataset = RenjiTacrolimusSurvivalDataset(
        root_dir=data_args.data_dir,
        split="train",
        shuffle=False,
        patient_subset_path=data_args.patient_subset_path,
    ...
    query_schema = dataset.task_schema["tacrolimus_abnormal_survival"]
    query_texts = {}
    ...
    config = LongTableEncoder1DConfig(
        ...
        stage_bins=[spec["num_bins"] for spec in STAGE_SPECS],
    )
    ...
    if model_args.pretrained_path:
        model = load_model_weights(model, model_args.pretrained_path)
    if model_args.use_lora:
        from peft import PeftModel

        model = PeftModel.from_pretrained(
            model, data_args.checkpoint_dir, is_trainable=False
        )
    else:
        model = load_model_weights(model, data_args.checkpoint_dir)
