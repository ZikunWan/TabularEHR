...
class EHRShotTestArguments:
    ...
    root_dir: str = field(default="./data/ehrshot", metadata={"help": "Root directory for EHRShot data."})
    ...
    task_name: Optional[str] = field(default=None, metadata={"help": "Optional single task filter. Required in sequence-classification mode."})
...
def _load_test_dataset(script_args: EHRShotTestArguments):
    sample_info_path = script_args.sample_info_path or os.path.join(script_args.root_dir, "index", "ehrshot_test.csv")
...
def main():
    parser = HfArgumentParser(EHRShotTestArguments)
    (script_args,) = parser.parse_args_into_dataclasses()

    print("=" * 80)
    print("EHRShot LLM Test")
    print("=" * 80)
    ...
    if script_args.use_sequence_classification:
        if not script_args.task_name:
            raise ValueError("Sequence classification mode requires --task_name for EHRShot evaluation.")
        _, candidates, label_to_id, id_to_label = _build_label_metadata(script_args.task_name)
        ...
        for index, output in enumerate(outputs):
            ...
            task_name = str(sample["task_info"]["task"])
