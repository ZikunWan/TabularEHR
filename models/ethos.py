...
GPTLMNoBiasModel = GPT2LMNoBiasModel


class GPT2NoBiasForSequenceClassification(nn.Module):
    def __init__(
        self,
        config: GPT2Config,
        num_labels: int,
        ...
        self.problem_type = getattr(config, "problem_type", None)

        self.backbone = GPT2LMNoBiasModel(config)
        dropout = config.resid_pdrop if classifier_dropout is None else classifier_dropout
        self.dropout = nn.Dropout(dropout)
        self.classifier = nn.Linear(config.n_embd, self.num_labels)

    @staticmethod
    def _pool_last_token(hidden_states: torch.Tensor, attention_mask: torch.Tensor | None):
    ...
    def forward(
        ...
        hidden_states = self.backbone.encode(input_ids)
        pooled = self._pool_last_token(hidden_states, attention_mask=attention_mask)
        ...
        if labels is not None:
            ...
            if problem_type is None:
                ...
                elif labels.dtype in (torch.long, torch.int, torch.int32, torch.int64):
