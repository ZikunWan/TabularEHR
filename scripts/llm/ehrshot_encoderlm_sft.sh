...
export WANDB_PROJECT=ehrshot_encoderlm
...
do
    TASK_KEY=$TASK_NAME
    OUTPUT_DIR="/data/zikun_workspace/checkpoints/ehrshot/${TASK_KEY}/table_only/gatortron_base_2k"
    RUN_NAME="ehrshot_${TASK_KEY}_table_only_gatortron_base_encoderlm_finetune"
    ...
    accelerate launch --num_processes="$NUM_GPUS" train_ehrshot_encoderLM.py \
